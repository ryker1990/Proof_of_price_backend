...
router.post('/api/signup', [
    body('email', 'All fields are required.').not().isEmpty().isEmail().withMessage('Please enter a valid email.'),
    body('password').not().isEmpty().withMessage('All fields are required.').isLength({
        min: 6
    }).withMessage('Password should be at least 6 characters.'),
    body('zipCode', 'All fields are required.').not().isEmpty()
], userController.signup)

router.post('/api/signin', [
    body('email').not().isEmpty().withMessage('All fields are required.').isEmail().withMessage('Please enter a valid email.'),
